...
item* slabs_cache_evict(unsigned int id) {
    ...
    // p->clock_max 는 perslab * slabs 
    // 최대 몇개가 있는지  
    assert(p->clock < p->clock_max);

    //??????????????????????????????????????????????????
    //뭐부터 시작한다는 거지, 난 이 조건이 이해가 안 가는데?
    //8번 안에 무조건 종료되는 거 아닌가?
    //0, 8, 16, 24, 이런 clock은 건너뛴다는 건데 
    while ((p->clock & 0x7) != 0) {
        //1이면 0으로 0이면 1으로 
    ...
    if (!foundzero) {
        ...
        //bitmap 중에 64bit만 가져온건가, 그리고 clock / 8 만큼  
        ...
        for (size_t steps = 0; steps < max_steps; steps ++) {
            ...
            if (*val64 == (uint64_t) -1) {
                *val64 = 0;
                val64 ++;   //이건 말이 안 되는데? 주소를 ++ 한다고?
                p->clock += 64; //한 꺼번에 건너 뛰어
                if (p->clock + 64 >= p->clock_max)
                    p->clock = 0;
                continue;
            }
